...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-[#070708] text-white overflow-x-hidden">
      ...
      <section className="py-16 px-6 border-t border-white/5">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-8">
            ...
            <h2 className="text-3xl font-bold">Intelligence feed, RAG pipeline, and brief generator</h2>
          </div>
          ...
          {/* RAG pipeline strip */}
          <div className="bg-[#0D0D0E] border border-[#D4A843]/15 rounded-xl p-6 flex items-start gap-6">
            <div className="w-10 h-10 rounded-lg bg-[#D4A843]/10 border border-[#D4A843]/20 flex items-center justify-center flex-shrink-0 mt-0.5">
              <Database className="w-5 h-5 text-[#D4A843]" />
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="font-bold text-white">RAG Document Pipeline</h3>
                <span className="text-[9px] font-bold text-sky-400 bg-sky-900/30 border border-sky-700/30 px-1.5 py-0.5 rounded uppercase tracking-widest">New · v2</span>
              </div>
              <p className="text-sm text-zinc-400 mb-4 leading-relaxed max-w-2xl">
                Upload regulatory PDFs, DOCX, or TXT files — the system chunks and embeds them with OpenAI, then lets you query across the entire corpus using natural language backed by Claude.
              </p>
              <div className="flex flex-wrap gap-2">
                {[
                  { href: "/upload", icon: Upload, label: "Upload Document" },
                  { href: "/documents", icon: Database, label: "Document Library" },
                  { href: "/query", icon: Search, label: "Query Engine" },
                ].map(({ href, icon: Icon, label }) => (
                  <Link
                    key={href}
                    href={href}
                    className="flex items-center gap-1.5 text-xs border border-[#D4A843]/30 text-[#D4A843] px-3 py-1.5 rounded-lg hover:bg-[#D4A843]/10 transition-all"
                  >
      ...
      {/* Custody module highlight */}
      <section className="py-16 px-6 border-t border-white/5">
        <div className="max-w-5xl mx-auto">
          <div className="bg-gradient-to-br from-[#D4A843]/8 via-transparent to-transparent border border-[#D4A843]/25 rounded-2xl p-8 flex items-start gap-8">
            <div className="w-12 h-12 rounded-xl bg-[#D4A843]/15 border border-[#D4A843]/30 flex items-center justify-center flex-shrink-0">
              <Lock className="w-6 h-6 text-[#D4A843]" />
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                <h3 className="text-xl font-bold text-white">Custody Intelligence — Digital Asset Custody Layer</h3>
                <span className="text-[9px] font-bold text-emerald-400 bg-emerald-900/30 border border-emerald-700/30 px-1.5 py-0.5 rounded uppercase tracking-widest">New</span>
              </div>
              <p className="text-zinc-400 text-sm leading-relaxed mb-5 max-w-2xl">
                Built for qualified custodians like Fireblocks Trust Company. Evaluate crypto assets against security and resiliency frameworks, track NYDFS/SEC/OCC/FinCEN custody regulations, and generate custody-specific compliance briefs — all in one place.
              </p>
              <div className="grid grid-cols-3 gap-3 mb-5">
                {[
                  { label: "Asset Evaluation Matrix", desc: "Security & resiliency scoring for BTC, ETH, SOL, USDC and more" },
                  { label: "Custody Regulatory Feed", desc: "NYDFS, SEC, OCC, FinCEN, CFTC, IRS — custody-specific rules only" },
                  { label: "NYDFS Compliance Tracker", desc: "BitLicense & Limited Purpose Trust Company charter requirements" },
                ].map(({ label, desc }) => (
              ...
              <Link
                href="/custody"
                className="inline-flex items-center gap-2 bg-[#D4A843] text-black font-bold px-5 py-2.5 rounded text-sm hover:bg-[#F5C842] transition-all"
              >
                Open Custody Intelligence <ChevronRight className="w-4 h-4" />
              </Link>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              { icon: Lock, href: "/custody", label: "Custody Intelligence", desc: "Asset evaluation & digital asset custody regulatory feed" },
              { icon: Database, href: "/documents", label: "Document Library", desc: "RAG corpus — upload, index, and manage regulatory documents", badge: "RAG" },
              { icon: Upload, href: "/upload", label: "Upload Document", desc: "Ingest PDFs, DOCX, or TXT into the vector search corpus", badge: "RAG" },
              { icon: Search, href: "/query", label: "Query Engine", desc: "Natural language search across the full document corpus", badge: "AI" },
              { icon: TrendingUp, href: "/reports", label: "Jurisdiction Mapping", desc: "Monitor regulatory activity across 15+ jurisdictions" },
            ].map(({ icon: Icon, href, label, desc, badge }) => (
              <Link key={label} href={href} className="bg-[#0D0D0E] border border-white/5 rounded-xl p-4 hover:border-[#D4A843]/30 hover:bg-[#D4A843]/5 transition-all group relative">
                {badge && (
                  <span className="absolute top-3 right-3 text-[9px] bg-[#D4A843]/20 text-[#D4A843] border border-[#D4A843]/20 px-1.5 py-0.5 rounded font-bold uppercase tracking-widest">
                    {badge}
                  </span>
                )}
